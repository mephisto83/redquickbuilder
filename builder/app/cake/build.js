...
function createElectronIO() {
    ...
    return Promise.resolve()
        ...
        .then(() => {
            console.log('installing fontawesome');
            console.log(path.join(localDir, appName));
            return executeSpawnCmd('yarn', ['add', 'red-react-payment-inputs'], {
                shell: true,
                cwd: path.join(localDir, appName)
            });
        })
...
function createWorkSpace() {
    ...
    return Promise.resolve()
        ...
        .then(() => {
            ...
            const testProject = `${build.solutionName}.Tests/${build.solutionName}.Tests.csproj`;
            const webProject = `${build.solutionName}.Web/${build.solutionName}.Web.csproj`;
            const projects = [
            ...
            projects.map((project) => {
                promise = promise.then(() =>
                    executeSpawnCmd(
                        'dotnet', ['add', project, 'package', 'RedQuick'], // , '-s', source
                        {}
                    )
                );
                promise = promise.then(() =>
