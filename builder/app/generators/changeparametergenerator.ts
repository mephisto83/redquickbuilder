...
export default class ChangeParameterGenerator {
	...
	static Generate(options: any) {
		...
		const agents = models.filter((x: any) => GetNodeProp(x, NodeProperties.IsAgent));
			...
			agents.map((agent: { id: any }) => {
				...
				afterEffects
					.filter((afterEffect: { id: any }) => {
						...
						const setup = GetNodeProp(afterEffect, NodeProperties.AfterMethodSetup);
						if (setup && functionType && setup[functionType][FunctionTemplateKeys.ModelOutput]) {
							...
							if (method) {
								let valid =
									GetMethodNodeProp(method, setup[functionType][FunctionTemplateKeys.Agent]) ===
										agent.id &&
									GetMethodNodeProp(method, setup[functionType][FunctionTemplateKeys.ModelOutput]) ==
										model.id;
								return valid;
							}
						}
					...
					.filter((afterEffect: { id: any }) => {
						...
						if (AFTER_EFFECTS[functionType] && AFTER_EFFECTS[functionType]) {
							const methodNode = GraphMethods.GetMethodNode(state, afterEffect.id);
				...
				const staticDic = {
					property_set_merge: '',
					model: `${GetNodeProp(model, NodeProperties.CodeName)}ChangeBy${GetNodeProp(
						agent,
				...
				testClass = bindTemplate(testClass, {
					name: `${GetNodeProp(model, NodeProperties.CodeName)}ChangeBy${GetNodeProp(
						agent,
						NodeProperties.CodeName
					)}`,
					tests: tests.unique((x: any) => x).join('')
				});

				const change_param_name = `${GetNodeProp(model, NodeProperties.CodeName)}ChangeBy${GetNodeProp(
					agent,
					NodeProperties.CodeName
				)}`;
				result[change_param_name] = {
					...
					tname: `${GetNodeProp(model, NodeProperties.CodeName)}ChangeBy${GetNodeProp(
						agent,
						NodeProperties.CodeName
					)}Tests`,
