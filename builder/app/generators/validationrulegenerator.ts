...
export default class ValidationRuleGenerator {
	static enumerateValidationTestVectors(validation_test_vectors: any[]) {
		var vects = validation_test_vectors.map((x: { values: { cases: {} } }) => {
	...
	static enumerate(vects: any, j = 0) {
		var results = [];
	...
	static GenerateValidationCases(graph: Graph, validatorNode: any) {
		...
		var validation_test_vectors: any = [];
                ...
                ${attribute_type_arguments.join(', ')}
						...
						case NodeTypes.Enumeration:
							if (validators && validators.enumeration) {
								...
								attribute_type_arguments = Object.keys(validators.enumeration)
									.map((ext) => {
		...
		let testProps = vectors.map((vector, index) => {
			...
			let propertyInformation: { set_properties: string; property: any; type: any }[] = [];
			let properylines = vector
				.map((v, vindex) => {
					...
					if (typeof projected_value === 'function') {
						...
						if (
							validation_test_vectors[vindex] &&
							validation_test_vectors[vindex].values &&
							validation_test_vectors[vindex].values.invalid &&
							!validation_test_vectors[vindex].values.invalid[_case]
						) {
							_case = '$$';
						}
					}
					...
					let propline =
						ValidationRuleGenerator.Tabs(3) +
						`{{model}}.${validation_test_vectors[vindex].property} = ${projected_value};`;
	...
	static Generate(options: { state: any; key: any }) {
		...
		nodes.map((node: any) => {
			...
			var modelNode: any = GraphMethods.GetNode(graph, model);
			var funct: any = GetNodeProp(node, NodeProperties.ValidatorFunction);
			var functNode = GraphMethods.GetNode(graph, funct);
			var validator = GetNodeProp(node, NodeProperties.Validator);
			let validatorProperties = GraphMethods.getValidatorProperties(validator);
			var validation_test_vectors: any = [];
			let propertyValidationStatements = Object.keys(validatorProperties)
				.map((property) => {
					let propertyNode = GraphMethods.GetNode(graph, property);
					let validatorPs = validatorProperties[property];

					return Object.keys(validatorPs.validators)
						.map((vld) => {
							let validators = validatorPs.validators[vld];
							let node = GraphMethods.GetNode(graph, validators.node);
							let attribute_type_arguments: any = '';
