...
const CONTROLLER_CLASS_FUNCTION_GET_TEMPLATE =
  "./app/templates/controller/controller_functions_get.tpl";
...
export default class ControllerGenerator {
  ...
  static Generate(options) {
    ...
    let _controllerTemplateClass = fs.readFileSync(
      CONTROLLER_CLASS_TEMPLATE,
      "utf8"
    );
    let _controllerTemplateFunction = fs.readFileSync(
      CONTROLLER_CLASS_FUNCTION_TEMPLATE,
      "utf8"
    );
    let _controllerTemplateFunctionGet = fs.readFileSync(
      CONTROLLER_CLASS_FUNCTION_GET_TEMPLATE,
      "utf8"
    );
    ...
    controllers.map(controller => {
      ...
      let maestros = GraphMethods.getNodesByLinkType(root, {
        ...
        if (maestro_functions.length) {
          maestro_functions
            .filter(
              x => !GetNodeProp(x, NodeProperties.NotIncludedInController)
            )
            .map(maestro_function => {
              var ft =
                MethodFunctions[
                  GetNodeProp(maestro_function, NodeProperties.FunctionType)
                ];
              if (ft) {
                ...
                if (!ft.controller)
                  if (
                    [Methods.Get, Methods.GetAll].some(
                      v =>
                        v ===
                        GetNodeProp(maestro_function, NodeProperties.MethodType)
                    )
                  ) {
                ...
                let modelNode = GraphMethods.GetNode(
                  root,
                  methodProperties.model
                );
                let fetch_parameter = GraphMethods.GetNode(
                  root,
                  methodProperties.fetch_parameter
                );
                ...
                tempFunction = bindTemplate(tempFunction, {
                  ...
                  http_method:
                    httpMethod || "{controller_generator_http_method",
