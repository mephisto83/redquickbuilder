...
const CONTROLLER_CLASS_FUNCTION_TEMPLATE =
  "./app/templates/controller/controller_functions.tpl";
const TEST_CLASS = "./app/templates/tests/tests.tpl";
const MAESTRO_FUNCTION_TESTS = "./app/templates/maestro/tests/maestro.tpl";
const MAESTRO_FUNCTION_SAME_AGENT_MODEL_TESTS =
  "./app/templates/maestro/tests/maestro_same_agent_model.tpl";
const get_agent_manytomany_listchild_interface =
  "./app/templates/maestro/tests/get_agent_manytomany_listchild_interface.tpl";
const MAESTRO_FUNCTION_GET_TESTS =
  "./app/templates/maestro/tests/maestro_get.tpl";
        ...
        return await ${GetCodeName(
          dataChainNode
              ...
              if (methodProps) {
                ...
                modelNode = GraphMethods.GetNode(
                  graphRoot,
                  methodProps[FunctionTemplateKeys.Model]
                );
                userTypeNode = GraphMethods.GetNode(
                  graphRoot,
                  methodProps[FunctionTemplateKeys.User]
                );
                ...
                model_output = GetCodeName(
                  methodProps[FunctionTemplateKeys.ModelOutput]
                );
                parentNode = GraphMethods.GetNode(
                  graphRoot,
                  methodProps[FunctionTemplateKeys.Parent]
                );
              ...
              let bindOptions = {
                ...
                output_type: modelNode
                  ? GetCodeName(modelNode)
                  : "{maestro_generator_missing_model}",
                  ...
                  cases.map(_case => {
                    ...
                    if (validator && validator.agent && _case.agentProperties) {
                      var temp = [
                        ...
                        ..._case.properties
                          .filter(x => x.type === FunctionTemplateKeys.Agent)
                          .map(t => t.props)
                          .map((t, index) => {
                            if (
                              validator.agent.propertyInformation.findIndex(
                                x => x.property === t.property
                              ) !== -1
                            ) {
                              return false;
                            }
                    ...
                    if (validator && validator.model && _case.itemProperties) {
                      var temp = [
                        ...
                        ..._case
                          .filter(x => x.type === FunctionTemplateKeys.Model)
                          .map(t => t.props)
                          .map((t, index) => {
                            if (
                    ...
                    if (validator && validator.agent && _case.agentProperties) {
                      var temp = [
                        ..._case.agentProperties.map(
                          (t, index) => _case.agentProps[index]
                        ),
                    ...
                    switch (function_type) {
                      case FunctionTypes.Get_ManyToMany_Agent_Value__IListChild:
                        templ = fs.readFileSync(
                          get_agent_manytomany_listchild_interface,
                          "utf8"
                        );
                        break;
                    }
