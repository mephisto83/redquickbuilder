...
let func = function(args = { viewType }) {
  // node0,node1
  //
  if (!args.viewType) {
  ...
  let { viewType } = args;
  let viewModelNode = GetComponentExternalApiNode(
  ...
  let model = GetNodesLinkedTo(graph, {
    ...
    [NodeTypes.Model].some(v => v === GetNodeProp(x, NodeProperties.NODEType))
  );
