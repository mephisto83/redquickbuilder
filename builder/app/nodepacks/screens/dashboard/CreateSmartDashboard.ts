...
export default function CreateSmartDashboard(args: SmartDashbordParmater) {
  ...
  if (args.buttons && args.buttons.length) {
    ...
    result.push(function(graph: Graph) {
      ...
      const layout: any = SetLayoutComponent(
        GetNodeById(screenOption, graph),
        mainSection,
        viewComponent
      );
      ...
      ...args.buttons.map((button: ButtonDescription) => {
        return function() {
          return AddComponent({
            ...
            uiType: args.uiType,
            componentType:
              ComponentTypes.ReactNative[ComponentTypeKeys.Button].key,
    ...
    result.push(
      ...args.buttons.map((button: ButtonDescription) => {
        return function() {
          ...
          throw new Error(
            'external label api is missing in create smart dashboards'
          );
        };
      })
    );
    ...
    result.push(function(graph: Graph) {
      ...
      args.buttons.forEach((button: ButtonDescription, index: number) => {
        if (button.id && button.buttonId) {
          layout = SetLayoutComponent(
            GetNodeById(viewComponent, graph),
            button.buttonId,
            button.id
          );
        } else {
