...
export function GetValidationMethodForViewTypes(node: any) {
	...
	return NodesByType(null, NodeTypes.Method)
		.filter((x: any) =>
			[ methodType ].some(
				(meth) => meth === (MethodFunctions[GetNodeProp(x, NodeProperties.FunctionType)] || {}).method
			)
		)
		.filter((x: any) => {
			if (viewTypeModel) {
