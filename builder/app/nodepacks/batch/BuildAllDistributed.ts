...
const Wait_For_Create_Component_All_Completion =
...
export const Add_Component_To_Screen_Options =
  'Add Component To Screen Options';
...
const Collection_DataChainsRelatedToMenuSource =
  'Add Data Chains related to menu source';
export const CollectionScreenWithoutDatachainDistributed =
...
export const CollectionConnectDataChainCollection =
...
export default async function BuildAllDistributed(
  command: string,
  currentJobFile: JobFile
) {
  ...
  try {
    ...
    await run(buildAllProgress, AddTitleService, async (progressFunc: any) => {
      graphOperation([addTitleService({ newItems: {} })])(
        GetDispatchFunc(),
        GetStateFunc()
      );
    });
    ...
    await run(
      buildAllProgress,
      Modify_Agent_Methods,
      async (progressFunc: any) => {
        await ModifyAgentMethods(progressFunc);
      }
    );
    await run(
      buildAllProgress,
      Add_Agent_Access_Methods,
      async (progressFunc: any) => {
    ...
    await run(
      ...
      async (progresFunc: (arg0: number) => any) => {
        ...
        await Object.keys(uiTypes).forEachAsync(
          async (uiType: any, uiIndex: number) => {
            graphOperation(
              CreateDashboard({
    ...
    await run(
      buildAllProgress,
      Add_Filters_To_Get_All,
      async (progresFunc: any) => {
    ...
    await run(
      buildAllProgress,
      Add_Chain_To_Navigate_Next_Screens,
      async (progresFunc: any) => {
    ...
    await run(
      ...
      async (progresFunc: any) => {
        graphOperation(CreateConfiguration())(
          GetDispatchFunc(),
          GetStateFunc()
        );
      }
    ...
    await run(
      ...
      async (progresFunc: (arg0: number) => any) => {
        graphOperation(CreateFetchServiceIdempotently())(
          GetDispatchFunc(),
          GetStateFunc()
        );
    ...
    await run(
      buildAllProgress,
      Collect_Screen_Connect_Into_Graph,
      async (progresFunc: (arg0: number) => any) => {
        await JobService.CollectForJob(currentJobFile);
      }
    );
    await run(
      ...
      Modify_Update_Links,
      async (progresFunc: (arg0: number) => any) => {
        await graphOperation(ModifyUpdateLinks())(
          GetDispatchFunc(),
          GetStateFunc()
        );
      }
    ...
    await run(
      buildAllProgress,
      Have_All_Properties_On_Executors,
      async (progresFunc: any) => {
    ...
    await run(
      buildAllProgress,
      Collection_SharedMenuDataSource,
      async (progressFunc: any) => {
    ...
    await run(
      buildAllProgress,
      Collection_DataChainsRelatedToMenuSource,
      async (progressFunc: any) => {
