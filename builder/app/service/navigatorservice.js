import { GetCodeName, GetNodeTitle, GetNodeProp, NodeProperties, GetConnectedScreenOptions, GetScreenOptions, GetNodeCode, GetJSCodeName, GetConnectedScreen } from "../actions/uiactions";
import fs from 'fs';
...
export function GenerateNavigationActions(language) {
    let options = GetScreenOptions().filter(x => GetNodeProp(x, NodeProperties.UIType) === language);
...
export function GenerateRoutes(language) {
    ...
    options.map(op => {
        routes = {
            ...routes,
            [GetCodeName(op)]: `${language === UITypes.ElectronIO ? '/' : ''}${GetNodeProp(op, NodeProperties.HttpRoute)}`
        };
    });
    return [{
