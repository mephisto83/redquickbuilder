import { GetServiceInterfaceMethodCalls, GetCodeName, GetCurrentGraph, GetNodeProp, GetMethodNode, GetState, GetMethodParametersFor } from "../actions/uiactions";
...
`
export function GenerateServiceInterface(serviceInterface, language) {
    ...
    methodCalls.map(mc => {
        let connectedNodes = GetNodesLinkedTo(graph, {
            id: mc.id
        }).filter(x => [NodeTypes.Permission, NodeTypes.Validator, NodeTypes.Executor].some(t => t === GetNodeProp(x, NodeProperties.NODEType)));
