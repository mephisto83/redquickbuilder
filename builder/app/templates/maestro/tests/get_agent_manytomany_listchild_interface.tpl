...
{{set_model_properties}}
            model = await modelArbiter.Create(model);
            
            var manyToMany = {{many_to_many}}.Create();
{{set_many_to_many_properties}}
            manyToMany.{{parent}} = agent.Id;
            manyToMany.{{model}} = model.Id;
            ...
            try 
            {
                ...
                var list = await maestro.{{function_name}}(user, agent.Id);
