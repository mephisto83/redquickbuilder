...
export function GetC (state, key, id) {
  if (state) { if (state.uiReducer && state.uiReducer[key]) return state.uiReducer[key][id] }
  return null
}

export function GetK (state, key, id, instance) {
  if (state) {
    if (state.uiReducer && state.uiReducer[key]) { if (state.uiReducer[key][id]) return state.uiReducer[key][id][instance] }
  }
  return null
}
