			...
			<TabPane active={active}>
					...
					<FormControl>
						...
						{hasLogicalChildren ? (
							<SelectInput
								...
								onChange={(value) => {
									...
									var types =
										UIA.GetNodeProp(currentNode, UIA.NodeProperties.LogicalChildrenTypes) || [];
									this.props.graphOperation([
										{
											operation: UIA.CHANGE_NODE_PROPERTY,
											options: {
						...
						{hasLogicalChildren ? (
							<ButtonList
								...
								items={(UIA.GetNodeProp(currentNode, UIA.NodeProperties.LogicalChildrenTypes) || [])
									.map((t) => {
						...
						{hasLogicalNieces ? (
							<SelectInput
								...
								onChange={(value) => {
									let id = currentNode.id;
									var types =
										UIA.GetNodeProp(currentNode, UIA.NodeProperties.LogicalNieceTypes) || [];
									this.props.graphOperation([
										{
											operation: UIA.CHANGE_NODE_PROPERTY,
											options: {
						...
						{hasLogicalNieces ? (
							<ButtonList
								...
								items={(UIA.GetNodeProp(currentNode, UIA.NodeProperties.LogicalNieceTypes) || [])
									.map((t) => {
										let node = GetNode(UIA.GetCurrentGraph(state), t);
										if (node) {
											return {
				...
				{currentNode ? (
					<ButtonList
						...
						items={(UIA.GetNodeProp(currentNode, UIA.NodeProperties.ManyToManyNexusTypes) || [])
							.map((t) => {
								let node = GetNode(UIA.GetCurrentGraph(state), t);
								if (node) {
									return {
