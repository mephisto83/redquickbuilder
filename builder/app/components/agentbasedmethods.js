        ...
        <FormControl>
          ...
          <div style={{ paddingBottom: 10 }}>
            <TreeViewMenu
              ...
              icon={'fa fa-times'}
              toggle={() => {
                this.props.graphOperation(UIA.NodesByType(state, NodeTypes.Model).map(model => {
                  return {
            ...
            <TreeViewMenu title={Titles.Models}
              ...
              {UIA.NodesByType(state, NodeTypes.Model).map(model => {
                return (
                  <TreeViewMenu key={`node--${model.id}`}
                    hideArrow={true}
                    title={UIA.GetNodeProp(model, NodeProperties.UIText)}
                    icon={!UIA.GetNodeProp(model, NodeProperties.AgentBasedMethod) ? "fa fa-circle-o" : 'fa fa-check-circle-o'}
                    toggle={() => {
          ...
          <button type="button" className="btn btn-block btn-warning btn-sm" onClick={() => {
            this.props.executeGraphOperations(UIA.NodesByType(state, NodeTypes.Model).filter(x => {
              ...
              let functionName = MethodFunctions[UIA.Visual(state, UIA.BATCH_FUNCTION_TYPE)].titleTemplate(UIA.GetNodeTitle(model), UIA.GetNodeTitle(UIA.GetNodeById(UIA.Visual(state, UIA.BATCH_AGENT))));
