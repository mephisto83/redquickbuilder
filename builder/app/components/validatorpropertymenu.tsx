	...
	render() {
		...
		if (currentNode && UIA.GetNodeProp(currentNode, UIA.NodeProperties.ValidatorModel)) {
			var propertyNodes = getNodesByLinkType(graph, {
				...
				.filter((x) => UIA.GetNodeProp(x, NodeProperties.NODEType) !== NodeTypes.Model)
				.map((t) => {
					return {
					...
					<FormControl>
						<SelectInput
							...
							onChange={(value) => {
								...
								var validator =
									UIA.GetNodeProp(currentNode, NodeProperties.Validator) || createValidator();
							...
							value={
								currentNode.properties ? currentNode.properties[UIA.NodeProperties.ValidatorModel] : ''
							}
						/>
					</FormControl>
				) : null}
