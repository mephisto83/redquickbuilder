      ...
      link.each((d) => {
        ...
        if (!me.avoidOverlaps) {
          d.route = Cola.makeEdgeBetween(
            createRectangle(d.source),
            createRectangle(d.target),
            5
          );
        } else {
          d.route = Cola.makeEdgeBetween(
...
function applyGroup(mindmapgroup, _group, groups, nodes) {
  if (_group) {
    if (_group.leaves && _group.leaves.length) {
      ...
      mindmapgroup.leaves.push(
        ..._group.leaves.map(l => nodes.findIndex(x => x.id === l))
      );
    } else {
    ...
    if (_group.groups && _group.groups.length) {
      ...
      mindmapgroup.groups.push(
        ..._group.groups.map(l => groups.findIndex(x => x.id === l))
      );
    } else {
