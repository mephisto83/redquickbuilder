...
class ValidatorPropertyActivityMenu extends Component<any, any> {
  render() {
    ...
    if (
      active &&
      currentNode &&
      UIA.GetNodeProp(currentNode, UIA.NodeProperties.ValidatorModel)
    ) {
      validator = UIA.GetNodeProp(currentNode, NodeProperties.Validator);
    }
    let propertyValidations = <div />;
    if (validator && validator.properties) {
      propertyValidations = Object.keys(validator.properties).map(key => {
        ...
        let selectedValidations = Object.keys(
          _validates && _validates.validators ? _validates.validators : {}
        ).map(v => {
            ...
            >
              <TreeViewMenu
                ...
                onClick={() => {
                  ...
                  let validator =
                    UIA.GetNodeProp(currentNode, NodeProperties.Validator) ||
                    createValidator();
        ...
        let validationUis = Object.keys(ValidationUI)
          .filter(
            x =>
              !_validates || !_validates.validators || !_validates.validators[x]
          )
          .reverse()
          .map(valiationUi => {
            return (
        ...
        return (
          ...
          >
            ...
            />
            <TreeViewMenu
              title={Titles.SelectedValidations}
              icon={"fa  fa-list-ul"}
      ...
      propertyValidations = (
        <div style={{ position: "relative" }}>
          <MainSideBar>
            <SideBar>
              <SideBarMenu>{propertyValidations}</SideBarMenu>
            </SideBar>
          </MainSideBar>
        </div>
      );
