					...
					<SideBarMenu>
						...
						{UIA.GetNodeProp(currentNode, NodeProperties.GroupParent) ? (
							<TreeViewMenu
								active={true}
								hideArrow={true}
								title={Titles.MergeChain}
								icon={'fa fa-code-fork '}
								onClick={() => {
									let groupProperties = UIA.GetNodeProp(currentNode, NodeProperties.GroupParent)
										? {
												id: UIA.getGroup(UIA.GetNodeProp(currentNode, NodeProperties.GroupParent))
													.id
											}
										: null;
									if (groupProperties)
							...
							<TreeViewMenu
								...
								onClick={() => {
									let groupProperties = UIA.GetNodeProp(currentNode, NodeProperties.GroupParent)
										...
										: null;
									if (groupProperties) {
										let groupExitNode = UIA.GetGroupProp(
											groupProperties.id,
											GroupProperties.GroupExitNode
										);
										let externalExitNode = UIA.GetGroupProp(
											groupProperties.id,
											GroupProperties.ExternalExitNode
										);
										if (externalExitNode) {
											this.props.graphOperation([
												...
												{
													operation: UIA.UPDATE_GROUP_PROPERTY,
													options: {
