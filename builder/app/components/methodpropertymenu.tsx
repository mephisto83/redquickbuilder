...
class MethodPropertyMenu extends Component<any, any> {
	render() {
		...
		if (functionObject && functionObject.constraints) {
			methodparams = Object.values(functionObject.constraints).map((constraint) => {
				let nodes = UIA.NodesByType(state, constraint.nodeTypes)
					.filter((node) => {
						return !Object.keys(constraint)
							.filter((x) => x != 'key' && x !== 'nodeTypes')
				...
				return (
					<SelectInput
						...
						value={
							UIA.GetNodeProp(currentNode, NodeProperties.MethodProps) ? (
								UIA.GetNodeProp(currentNode, NodeProperties.MethodProps)[constraint.key]
							) : (
								''
							)
						}
		...
		return <TabPane active={active}>{currentNode ? <FormControl>{methodparams}</FormControl> : null}</TabPane>;
	}
}
