...
class LayoutView extends Component<any, any> {
	...
	getComponentApi(cellChildren) {
		...
		if (cellChildren && cellChildren[selectedCell]) {
			...
			const componentApi = UIA.GetNodeProp(UIA.GetNodeById(childComponent), NodeProperties.ComponentApi);
			if (componentApi) {
				...
				const { instanceType, model, selector, handlerType, dataChain, modelProperty } =
				...
				return [
					...
					selectedComponentApiProperty && instanceType === InstanceTypes.Selector ? (
						<SelectInput
	...
	render() {
		...
		let componentNodes = currentNode
			? GetConnectedNodesByType(state, currentNode.id, NodeTypes.ComponentNode, SOURCE)
			: [];
		componentNodes = [
			...componentNodes,
		...
		const componentProperties = UIA.GetNodeProp(currentNode, UIA.NodeProperties.ComponentProperties);
		const componentPropertiesList = getComponentPropertyList(componentProperties);
		const onUpdateComponentTags = (item) => {
			const layout = UIA.GetNodeProp(currentNode, NodeProperties.Layout) || CreateLayout();
					...
					<div className="row">
						<div className="col-md-2">
							...
							<Box primary maxheight={350} title={Titles.Layout}>
								...
								{cellProperties && cellProperties.name && this.state.selectedCell ? (
									<TextInput
										immediate
										onChange={(val: any) => {
											const layout = nodeLayout || CreateLayout();
									...
									<ButtonList
										...
										isSelected={(item) => {
											if (cellProperties.properties) {
												return (cellProperties.properties.tags || [])
													.some((v) => v === item.value);
											}
											return false;
										}}
