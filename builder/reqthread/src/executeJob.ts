...
import { Graph, Node } from '../../app/methods/graph_types';
    ...
    if (partContent) {
      ...
      switch (command) {
        ...
        case BAD.Setup_View_Types:
          await SetupViewTypes(onProgress, (model: Node) => {
            return filter && filter.models.indexOf(model.id) !== -1;
          });
          break;
        case BAD.Connect_Dashboards:
          await ConnectDashboards((model: Node) => {
            return filter && filter.models.indexOf(model.id) !== -1;
          }, onProgress);
          break;
        case BAD.Build_Dashboards:
          await BuildDashboards((model: Node) => {
            return filter && filter.models.indexOf(model.id) !== -1;
          });
          break;
        case BAD.ApplyTemplates:
          await ApplyTemplates((model: Node) => {
            return filter && filter.models.indexOf(model.id) !== -1;
          });
          break;
